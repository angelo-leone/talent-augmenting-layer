---
name: talent-assess
description: Run a Talent-Augmenting OS onboarding assessment to build your personalised AI profile. Takes ~15 minutes of conversation. Use when a user wants to create or redo their profile.
allowed-tools:
  - Read
  - Write
  - Glob
  - Grep
  - Bash
  - mcp__plugin_talent-augmenting-layer_talent-augmenting-layer__talent_assess_start
  - mcp__plugin_talent-augmenting-layer_talent-augmenting-layer__talent_assess_score
  - mcp__plugin_talent-augmenting-layer_talent-augmenting-layer__talent_assess_create_profile
  - mcp__plugin_talent-augmenting-layer_talent-augmenting-layer__talent_suggest_domains
---
...
1. Call the MCP tool `talent_assess_start` to fetch the full question bank, scoring anchors, and domain taxonomy as JSON. This is the single source of truth: do NOT try to read `mcp-server/src/assessment.py` from the filesystem (plugin installs don't ship that file).
2. If `talent_assess_start` fails or times out after ~90 seconds, tell the user:
   > "The TAOS server is taking a while to wake up. Open https://proworker-hosted.onrender.com in a browser tab, wait ~45 seconds for it to respond, then tell me 'retry' and I'll try again."
