...
def get_assessment_protocol() -> dict:
    ...
    return {
        ...
        "instructions": (
            "# MANDATORY TOOL SEQUENCE — read this before anything else\n\n"
            "This assessment is not complete until BOTH of the following tool calls have "
            "returned success in this conversation:\n\n"
            "  1. `talent_assess_score(answers, domain_ratings)` — computes the user's scores.\n"
            "  2. `talent_assess_create_profile(name, scores, domain_ratings, ...)` — persists "
            "the profile.\n\n"
            "RULES YOU MUST FOLLOW:\n\n"
            "- You must collect a discrete integer answer (1 to 5) for EVERY question listed "
            "  in the `sections` block below. The `answers` argument to `talent_assess_score` "
            "  is a dict keyed by the question's `id` field (A1, A2, A3, A4, A5, B1, B2, B3, "
            "  B4, B5, D1, D2, D3, D4) with integer values. You cannot skip questions. You "
            "  cannot leave the user's answer in free-form prose without translating it into "
            "  one of the 1-5 anchors.\n"
            "- You must collect per-domain ratings (1 to 5) for at least 5 expertise domains. "
            "  Pass them as `domain_ratings`: a dict keyed by domain name with integer values.\n"
            "- You must NOT produce a final assessment summary, scores, ratings, or any "
            "  'preliminary' interpretation of the user's profile before BOTH tool calls have "
            "  returned success. If the user asks 'what do you think so far?' mid-assessment, "
            "  tell them you would rather complete the structured questions first so the "
            "  scoring function can produce calibrated results.\n"
            "- If `talent_assess_create_profile` returns an error, surface the exact error to "
            "  the user and retry. Do not silently abandon the save. Do not fabricate a "
            "  technical reason ('resource/path error', etc.) if the call was never attempted.\n"
            "- The assessment is incomplete if the user closes the conversation before "
            "  `talent_assess_create_profile` succeeds. Treat reaching the save step as the "
            "  load-bearing goal of the session.\n\n"
            "# Conversational flow\n\n"
            "The questions can be asked warmly and conversationally. The MANDATORY rules above "
            "are about the STRUCTURED data you collect and the TOOLS you call. The TONE is yours.\n\n"
            "1. IDENTITY: name, role, organisation, industry, context summary. (2-3 minutes)\n"
            "2. SECTION A (AI Dependency Risk): 5 questions, one at a time, behavioural "
            "   anchors 1-5. Record an integer per question. (3-4 minutes)\n"
            "3. SECTION B (Growth Potential): 5 questions, same shape. (3-4 minutes)\n"
            "4. SECTION D (AI Literacy): 4 questions, same shape. (2-3 minutes)\n"
            "5. EXPERTISE DOMAINS: ask which domains are most relevant to their work; rate "
            "   each 1-5 on the ESA scale with a behavioural example. Aim for 5-10 domains. "
            "   (3-5 minutes)\n"
            "6. GOALS & CONTEXT: career goals, skills to develop, skills to protect, task "
            "   classification (automate/augment/coach/protect/hands-off), red lines, learning "
            "   and feedback and communication style. (3-4 minutes)\n"
            "7. SCORE & SAVE: call `talent_assess_score` with the integer answers; then call "
            "   `talent_assess_create_profile` with scores plus qualitative data; only then "
            "   present the results and ask if anything feels off.\n\n"
            "# Style guidelines\n\n"
            "- Warm and conversational, not clinical. Explain WHY each question matters in one "
            "  short sentence.\n"
            "- If the user gives a vague answer, probe with the behavioural anchors.\n"
            "- It's fine to adjust an inferred score based on behavioural evidence (expert "
            "  interviewer mode), but you still record an integer 1-5 for every question.\n"
            "- Whole session: 15-20 minutes.\n"
            "- After the save tool returns success, present the scores and ask: 'Do these "
            "  feel accurate? Anything you'd adjust?'"
        ),
        "sections": {
            "A": {
