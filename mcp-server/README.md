...
> Use your Pro Worker AI profile with ANY MCP-compatible client.
...
- **Environment**: Set `PROWORKER_PROFILES_DIR` to your profiles directory
...
| `proworker://system-prompt/{name}` | Complete system prompt (CLAUDE.md + profile) ready for any LLM |
| `proworker://coaching-modules` | Structured coaching session modules |
...
Use the `proworker-system` prompt or `proworker_get_calibration` tool.

```
User: "Help me write an evaluation framework"
Claude: [calls proworker_get_calibration(name="Angelo")]
...
Use the `proworker://system-prompt/{name}` resource to get a complete system prompt, then paste it into ChatGPT, Gemini, or any other LLM's system/custom instructions.

### Pattern 4: Custom Agents
Build agents that call `proworker_classify_task` before acting:

```python
# Pseudocode for an agent that respects Pro Worker AI
task = user_input()
classification = mcp.call("proworker_classify_task", name="Angelo", task_description=task)
...
```python
mcp.call("proworker_log_interaction",
    name="Angelo",
    task_category="coach",
...
```python
stats = mcp.call("proworker_get_progression", name="Angelo")
...
|----------|---------|-------------|
| `PROWORKER_PROFILES_DIR` | `../profiles/` | Directory where profile `.md` files are stored |
