...
**What it is**: A full tool integration layer that exposes Talent-Augmenting Layer as 14 MCP tools, 5 resources, and 4 prompts. Provides automatic interaction tracking, server-side scoring, and programmatic profile management.
...
In this MCP tier, the conversation is generated by the user's selected client model (for example, Claude Code). The TAL MCP server provides prompts, tools, resources, and profile state.
