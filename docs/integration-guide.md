# Talent-Augmenting OS: Integration Guide

> How to use Talent-Augmenting OS with any LLM, agent, or tool.
...
All tiers share: same TAOSQ (14 fixed items + adaptive domains),
...
4. All conversations within the project use the Talent-Augmenting OS behaviour
...
**What it is**: A full tool integration layer that exposes Talent-Augmenting OS as 14 MCP tools, 5 resources, and 4 prompts. Provides automatic interaction tracking, server-side scoring, and programmatic profile management.
...
| `talent_assess_score` | `{answers, domain_ratings}` | Computed scores (ADR, GP, ALI, ESA, TAOSRI) |
...
| `talent-update` | Profile update workflow based on recent interaction patterns |

In this MCP tier, the conversation is generated by the user's selected client model (for example, Claude Code). The TAOS MCP server provides prompts, tools, resources, and profile state.
...
- Server-side scoring (TAOSQ scores computed by the assessment engine)
...
TAOSQ scores: ADR, GP, ALI, ESA, TAOSRI
...
1. New member takes the TAOSQ assessment (via any tier)
2. Profile is generated and saved to the shared profiles directory
3. Member configures their LLM/IDE with the Talent-Augmenting OS system prompt
...
| 12 | `talent_assess_score` | Compute TAOSQ scores from raw answers |
...
| `talent-update` | Profile update workflow |
